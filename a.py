...
pipe = StableDiffusionPipeline.from_pretrained(model_id).to(device).half()  # 使用半精度
...
num_inference_steps = st.slider("推理步数", 5, 30, 20)  # 减少推理步数
...
if st.button("生成图片"):
    with st.spinner("正在生成图片..."):
        ...
        # 生成随机噪声
        latents = torch.randn((1, 4, 32, 32), device=device).half()  # 使用更小的分辨率和半精度
